...
- **Stats go at the top of `Building`,** as an italic line: commits, PRs, repo count, and the tech/fund split.
  - **Lead with PRs, not commits.**  Use `prs_merged_publishable` (what landed that week) and `prs_open_now_publishable` (what's still in flight).  **Show both** — in-progress work is real work, and a weekly that only counts what shipped understates a week spent deep in something big.
  - **Commits are a soft number and should be labelled as one.**  Squash-merging collapses a 39-commit branch into one mainline commit, so `commits_publishable_primary` undercounts effort; `commits_publishable` (all refs) overcounts what landed *and* is machine-dependent, since a checkout with different local branches gives a different answer for the same closed week.  Neither is "how much work happened".  Publish the mainline figure if it's interesting, say plainly that squashes soften it, and never lead with it.  For W34: 677 all-refs, 511 mainline, 48 PRs — the PR number is the only one that means the same thing on every machine and under every merge strategy.
  - **`prs` (the subject-parsed list) is for naming PRs in a catchup, not for counting.**  It scrapes `#NN` from commit subjects, so it picks up cross-repo references, PRs merely mentioned, and duplicates.  For W34 it read 37 where GitHub said 24.
  - **Work repos only** (`lane` of `build` or `fund`).  The stat line sits under **Building**; the writing lane isn't building, and including it inflates the repo count.
  - **The PR split can disagree with the commit split, and that's information.**  W34 was 72/28 tech by commits and 24/24 by PRs — the tech PRs were simply much larger.  When they diverge, say so in a clause rather than picking the flattering one.  The gap between them is the personal lane, and publishing the delta turns it into a recurring public number — which is precisely the exposure the split exists to prevent.  Repo *count* is fine; repo *names* are not.
