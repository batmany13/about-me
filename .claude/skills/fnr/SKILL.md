...
- **Stats go at the top of `Building`,** as an italic line: commits, PRs, repo count, and the tech/fund split.
  - **Use `commits_publishable_primary` — mainline only.**  The all-refs figure counts commits sitting on unmerged worktree branches, which is both an overcount of what landed and *machine-dependent*: a different checkout with different local branches yields a different number for the same week.  A published statistic that changes depending on which laptop ran the script is not a statistic.  For W34 the two read 677 and 511 — a 166-commit gap, all of it in flight.
  ...
  - **Never `commits_all`.**  The gap between them is the personal lane, and publishing the delta turns it into a recurring public number — which is precisely the exposure the split exists to prevent.  Repo *count* is fine; repo *names* are not.
