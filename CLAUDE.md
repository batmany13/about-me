...
3. Write the whole week in one pass, **all of it in the private repo**: `drafts/<week>.unredacted.md` (names and numbers intact), the scrubbed candidate `drafts/<week>.public.md` with every owner block in `<!-- fnr:key -->` markers and filled with its machine default, and `drafts/<week>.delta.md` (what was held back, plus the flagged list)
4. Ask the owner **one question per turn**, showing the unredacted and scrubbed section together; each answer is written straight into the public file, verbatim, via `scripts/state.py` — never scrubbed, never edited afterwards. The sections, the owner's blocks, the one required block and the questions are declared in `fnr/.private/fnr.config.json`, not in the skill; the weekly does not publish until the required block is answered. `drafts/<week>.state.json` makes it resumable across sessions
5. Publish check on the flagged list. **Publish is the literal word from Bruce**, quoted into the state (`state.py publish --quote`); decisions on flagged items are not it, and until then nothing is written here. Only `state.py release` copies the candidate to `fnr/<week>.md`, on a branch + PR here; the private artifacts (unredacted, candidate, delta, state, rollup snapshot, next week's `wip`) go on a branch + PR in the private repo
6. Optionally, `substack.py <week>` copies the released, committed weekly to the clipboard as rich text for pasting into a new Substack post. Substack has no API that writes posts, so the paste and the Publish button stay Bruce's
